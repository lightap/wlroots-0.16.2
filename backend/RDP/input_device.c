...
static void send_key_event(struct wlr_RDP_backend *RDP, uint32_t key,
	...
	wlr_keyboard_notify_key(&RDP->keyboard, &ev);
}
...
static struct wlr_RDP_touchpoint *get_touchpoint_from_RDP_touch_id(
		struct wlr_RDP_output *output, uint32_t id) {
	struct wlr_RDP_touchpoint *touchpoint;
	wl_list_for_each(touchpoint, &output->touchpoints, link) {
		if (touchpoint->RDP_id == id) {
			return touchpoint;
		}
	}
	return NULL;
...
void handle_RDP_xinput_event(struct wlr_RDP_backend *RDP,
	...
	case XCB_INPUT_BUTTON_PRESS: {
		...
		output = get_RDP_output_from_window_id(RDP, ev->event);
		if (!output) {
			return;
		}
	...
	case XCB_INPUT_TOUCH_BEGIN: {
		...
		if (!wl_list_empty(&output->touchpoints)) {
			struct wlr_RDP_touchpoint *last_touchpoint = wl_container_of(
	...
	case XCB_INPUT_TOUCH_END: {
		...
		struct wlr_RDP_touchpoint *touchpoint = get_touchpoint_from_RDP_touch_id(output, ev->detail);
		if (!touchpoint) {
			return;
		}
...
void update_RDP_pointer_position(struct wlr_RDP_output *output,
	...
	xcb_query_pointer_cookie_t cookie =
		xcb_query_pointer(RDP->xcb, output->win);
	xcb_query_pointer_reply_t *reply =
		xcb_query_pointer_reply(RDP->xcb, cookie, NULL);
	if (!reply) {
		return;
	}
